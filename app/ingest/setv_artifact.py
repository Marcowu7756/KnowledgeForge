"""SETV Artifact adapters — snapshot / evolution / family (Export Contract v0).
...
AssetClass = Literal["snapshot", "evolution", "family"]
...
def ingest_artifact(
    ...
    *,
    ...
    dest = dest_dir or (config.KNOWLEDGE_DIR / "restricted" / "setv" / dest_leaf)
    stem = f"{asset_class}_{parsed.artifact_id.lower().replace('-', '_')}"
