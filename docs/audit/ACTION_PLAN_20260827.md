...
| **A5** | F-P2-02 EmbeddingRef 未落盘 | 中 | `data/retrieve/embeddings.json` sidecar | ✅ DONE |
...
- Deeper A8: contrast cross-KO clustering from explicit `vs` relation pairs
- Real Whisper wav fixture under `tests/integration/`
- Package-level EmbeddingRef write-back (still deferred; sidecar is SoT for now)
